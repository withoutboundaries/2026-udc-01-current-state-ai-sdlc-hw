...
Move one-time startup out of components. Call it from the app entry (root layout or client bootstrap) so remounts and Strict Mode never re-run side effects.
